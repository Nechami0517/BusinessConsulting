...
export const fetchOwnerMeetings = createAsyncThunk(
  'meetings/fetchOwnerMeetings',
  async (_, { rejectWithValue }) => {
    try {
      const meetings = await meetingsAPI.getOwnerMeetings();
      return meetings;
    } catch (error: any) {
      return rejectWithValue(error.response?.data?.message || 'Failed to fetch owner meetings');
    }
  }
);
...
const meetingsSlice = createSlice({
  ...
  extraReducers: (builder) => {
    builder
      ...
      // Fetch owner meetings
      .addCase(fetchOwnerMeetings.pending, (state) => {
        state.isLoading = true;
        state.error = null;
      })
      .addCase(fetchOwnerMeetings.fulfilled, (state, action) => {
        state.isLoading = false;
        state.meetings = action.payload;
        state.error = null;
      })
      .addCase(fetchOwnerMeetings.rejected, (state, action) => {
        state.isLoading = false;
        state.error = action.payload as string;
      })
